...
class TranscriptScreen extends Component {
  ...
  componentDidMount() {
    axios
      .get(`${API_ROOT}/api/speech/watson-data/${this.props.speech}`)
      .then(speech => {
        this.setState({
  ...
  thesaurus(word) {
    axios
      .get(`${API_ROOT}/api/speech/thesaurus/${word}`)
      .then(res => {
        //set state to first ten words returnd from the backend
        alternatives = res.data
          .slice(0, 10)
          .map(word => (
            <Text style={styles.transcriptAlternativesText}>{word}, </Text>
          ))
  ...
  renderSpeech() {
    ...
    const match = (pattern, value, start = 0) => {
      ...
      const matched = []
      for (let i = 0; i != end; ++i)
        if (pattern[i] !== value[start + i]) return false
        else if (pattern[i] === '%hesitation') matched.push('um')
        else matched.push(pattern[i])
      return matched
    }
        ...
        {this.state.alternatives.length ? (
          <Card>
            <TouchableOpacity
              ...
              onPress={() => this.setState({ alternatives: [] })}
            >
              <Text>X</Text>
            </TouchableOpacity>
            <Text style={styles.transcriptSelectedWord}>
              Synonyms for {this.state.selectedWord}
            </Text>
            <View style={styles.transcriptAlternative}>
        ...
        <Card containerStyle={styles.transcriptCardContainer}>
          <ScrollView>
            <View style={styles.transcriptCardContainerView}>
              {this.state.speech ? this.renderSpeech() : <Text />}
            </View>
          </ScrollView>
        </Card>
      </View>
